...
from PySide6.QtWidgets import (
    ...
    QDoubleSpinBox, QFileDialog, QMainWindow, QFrame, QSpacerItem, QSizePolicy, QScrollArea
)
...
class DeviceControlMainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # --- LEFT COLUMN: Controls (inside a scroll area so nothing is clipped) ---
        left_content = QWidget()
        main_layout = QVBoxLayout(left_content)
        ...
        self.enable_motors_btn = QPushButton("Enable Motors")
        self.enable_motors_btn.setCheckable(True)
        self.enable_motors_btn.setMinimumHeight(48)
        self.enable_motors_btn.setStyleSheet("""
            QPushButton { background-color: #555; color: white; border-radius: 4px; font-size: 14px; font-weight: bold; }
            QPushButton:checked { background-color: #27a060; }
            QPushButton:hover   { background-color: #666; }
            QPushButton:checked:hover { background-color: #32c070; }
        """)
        self.enable_motors_btn.clicked.connect(self.toggle_motors)
        main_layout.addWidget(self.enable_motors_btn)
        ...
        layout.addWidget(self.create_button("Spin Motor 3 (1 rev)", self.spin_motor_test, font), 8, 0, 1, 2)
        main_layout.addWidget(advanced_frame)

        # vertical stretch (pushes content to top inside the scroll area)
        main_layout.addStretch(1)

        # --- RIGHT COLUMN: Video Display ---
        self.video_viewer.setMinimumWidth(400)
    ...
    def spin_motor_test(self):
        if not self.oms.is_connected():
            return
        self.oms.spin_joint(joint_index=2, delta_deg=360.0, speed_dps=180.0)
    ...
    def toggle_motors(self, checked):
        if not self.oms.is_connected():
            self.enable_motors_btn.setChecked(False)
            return
        result = self.oms.enable_motors(checked)
        if result != SerialInterface.ReplyStatus.OK:
            # Command failed — reset button to match actual state
            self.enable_motors_btn.setChecked(not checked)
            return
        self.enable_motors_btn.setText("Disable Motors" if checked else "Enable Motors")
        if checked:
            # After M17, the firmware recalculates current_pose via forward kinematics
            # (FK of all-joints-zero ≠ Cartesian zero).  Re-read so GUI and firmware agree.
    ...
    def move_axis(self, axis, direction):
        ...
        prev = self.current_pos[axis]
        self.current_pos[axis] += direction * d * flipped[axis]
        self.current_pos[axis] = max(min(self.current_pos[axis], 20), -20)
        result = self.oms.move_to(*self.current_pos, self.feedrates[self.step_size_idx])
        if result != SerialInterface.ReplyStatus.OK:
            self.current_pos[axis] = prev  # revert if firmware rejected the move
