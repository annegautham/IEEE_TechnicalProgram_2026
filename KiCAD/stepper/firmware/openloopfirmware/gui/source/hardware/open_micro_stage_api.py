...
class OpenMicroStageInterface:
    ...
    def spin_joint(self, joint_index=2, delta_deg=360.0, speed_dps=180.0):
        """Directly spin one motor joint by delta_deg at speed_dps, bypassing kinematics."""
        if self.serial is None:
            return False
        cmd = f"M60 J{joint_index} A{delta_deg:.2f} S{speed_dps:.2f}"
        timeout = abs(delta_deg) / max(speed_dps, 1.0) + 5.0
        res, msg = self.serial.send_command(cmd, timeout=timeout)
        return res
